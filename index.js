...
const xs = sampleData.map(item => [Number(item.D)]);
const ys = sampleData.map(item => item.L);

const numberOfFutureDays = 10; 
trainModel(xs,ys,xs[0].length,5,0.01,5,()=>{
  console.log("done")
}).then((result) => {
  console.log(xs.slice(xs.length - numberOfFutureDays))
  console.log(makePredictions(xs,result.model,result.normalize).slice(xs.length - numberOfFutureDays))
});